...
tipo=reader-csv-single-postgre
csvdir=/etl/postgresql/
pidfile=/var/run/etlagentPostgre.pid
logfile=/var/log/etl/read-csv-single-postgre.log
javapath=/usr/java/jre1.7.0_21/bin/java
statusfile=/cassandra/etl-agent/agent/status/statusPostgre.conf
...
while [ -n "`ls $csvdir/*csv`" ];do 
  sleep 1
  [ "`cat $statusfile|cut -d'\"' -f2`" == "2" ] && echo "Ha habido un error segun el statusfile" && exit 1
  ...
  java -server -Xmx2048m -cp $CLASSPATH -Dlog4j.configuration=file:$AGENT_HOME/log4j/read-csv-single-postgre.properties agent.Main $AGENT_HOME/conf $tipo
done
